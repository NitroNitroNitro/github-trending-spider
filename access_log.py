...
class AccessLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        client_ip = _get_client_ip(request).replace("\r", "").replace("\n", "")
        ...
        user_agent = request.headers.get("user-agent", "未知客户端").replace("\r", "").replace("\n", "")
        ...
        with _stats_lock:
            ...
            if len(_stats["IP计数"]) < 1000 or client_ip in _stats["IP计数"]:
                _stats["IP计数"][client_ip] += 1
            if len(_stats["接口计数"]) < 1000 or path in _stats["接口计数"]:
                _stats["接口计数"][path] += 1
            if status_code >= 400:
                _stats["错误请求数"] += 1
