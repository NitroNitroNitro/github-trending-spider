...
class AccessLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        clean_method = method.replace('\r', '').replace('\n', '')
        clean_path = path.replace('\r', '').replace('\n', '')
        clean_ua = user_agent.replace('\r', '').replace('\n', '')

        # 输出访问日志（中文格式）
        logger.info(
            "[访问] 来源IP=%s | 请求=%s %s | 状态码=%d | 耗时=%dms | 客户端=%s",
            clean_ip, clean_method, clean_path, status_code, latency_ms, clean_ua,
        )
        ...
        with _stats_lock:
            ...
            if len(_stats["IP计数"]) < 10000 or clean_ip in _stats["IP计数"]:
                _stats["IP计数"][clean_ip] += 1
            if len(_stats["接口计数"]) < 10000 or clean_path in _stats["接口计数"]:
                _stats["接口计数"][clean_path] += 1
            if status_code >= 400:
                _stats["错误请求数"] += 1
