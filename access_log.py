...
class AccessLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # 🛡️ Sentinel: Sanitize inputs to prevent Log Injection
        client_ip = str(_get_client_ip(request)).replace("\n", " ").replace("\r", " ")
        method = str(request.method)
        path = str(request.url.path).replace("\n", " ").replace("\r", " ")
        ...
        user_agent = str(request.headers.get("user-agent", "未知客户端")).replace("\n", " ").replace("\r", " ")
        ...
        with _stats_lock:
            ...
            # 🛡️ Sentinel: Limit unique IPs and paths tracking to prevent memory exhaustion DoS
            if len(_stats["IP计数"]) < 50000 or client_ip in _stats["IP计数"]:
                _stats["IP计数"][client_ip] += 1
            if len(_stats["接口计数"]) < 5000 or path in _stats["接口计数"]:
                _stats["接口计数"][path] += 1
