...
def _process_hacker_news(errors):
...
def _process_linux_do(errors):
    ...
    try:
        ...
        linux_do_items = []
    return linux_do_items

def _process_v2ex(errors):
...
def _process_official_ai(errors):
    ...
    return ai_source_items

def _process_storage(content_items, errors):
    from content_items import write_content_json
...
def run_spider(scheduled_time=None):
    ...
    daily_repos, weekly_repos = _process_github(errors)
    hn_stories = _process_hacker_news(errors)
    linux_do_items = _process_linux_do(errors)
    v2ex_topics = _process_v2ex(errors)
    tldr_items = _process_tldr_ai(errors)
    ai_source_items = _process_official_ai(errors)
    ...
    _process_storage(content_items, errors)
