...
def list_recent_history_dates(days=7, output_dir=OUTPUT_ARCHIVE_DIR, today=None):
    ...
    for offset in range(1, days + 1):
        ...
        for source in SOURCE_DEFINITIONS:
            ...
            if not batch_file:
                continue
            # ⚡ Bolt: Use fast regex extraction instead of full json.load() to prevent performance bottlenecks
...
def _read_item_count_fast(path):
    """
    Fast extraction of item_count from JSON snapshot using regex on the first chunk.
    Falls back to full JSON parsing if not found.
    """
    try:
        with path.open("r", encoding="utf-8") as f:
            chunk = f.read(2048)
            # ⚡ Bolt: Use regex to quickly extract item_count from large JSON files
            # to avoid expensive full json.load() in list_recent_history_dates.
            match = re.search(r'\n  "item_count":\s*(\d+)', chunk)
            if match:
                return int(match.group(1))
    except OSError as e:
        logger.warning("Fast read item_count chunk failed: %s", e)
        return 0

    # Fallback to full parse if regex misses
