...
ITEM_COUNT_REGEX = re.compile(rb'"item_count"\s*:\s*(\d+)')
...
def list_recent_history_dates(days=7, output_dir=OUTPUT_ARCHIVE_DIR, today=None):
    ...
    for offset in range(1, days + 1):
        ...
        for source in SOURCE_DEFINITIONS:
            ...
            if not batch_file:
                continue
            # ⚡ Bolt: Use fast path to avoid full JSON parsing for just item_count
...
def _fast_read_item_count(path):
    """Fast path to extract item_count without parsing the entire JSON file."""
    try:
        with path.open("rb") as f:
            chunk = f.read(2048)
            match = ITEM_COUNT_REGEX.search(chunk)
            if match:
                return int(match.group(1))

            f.seek(0)
            return json.loads(f.read().decode("utf-8")).get("item_count", 0)
    except Exception as e:
        logger.warning("快速读取归档 item_count 失败: %s", e)
        return 0
