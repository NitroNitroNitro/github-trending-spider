...
def list_recent_history_dates(days=7, output_dir=OUTPUT_ARCHIVE_DIR, today=None):
    ...
    for offset in range(1, days + 1):
        ...
        for source in SOURCE_DEFINITIONS:
            ...
            if not batch_file:
                continue
            item_count = _fast_extract_item_count(batch_file)
            sources.append({
                "source": source,
                "source_id": source["id"],
...
def _fast_extract_item_count(path):
    """Fast extraction of item_count from snapshot JSON without full parse."""
    try:
        ...
        logger.warning("Regex parsing item_count failed: %s", e)
