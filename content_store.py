...
def list_recent_history_dates(days=7, output_dir=OUTPUT_ARCHIVE_DIR, today=None):
    ...
    for offset in range(1, days + 1):
        ...
        for source in SOURCE_DEFINITIONS:
            ...
            # ⚡ Bolt: Fast regex extraction to avoid loading entire JSON files for just the item count
            item_count = _extract_item_count_fast(batch_file)
            sources.append({
                "source": source,
                "source_id": source["id"],
...
def _extract_item_count_fast(path):
    """Extract item_count without loading the entire JSON."""
    try:
        with path.open("r", encoding="utf-8") as f:
            chunk = f.read(2048)
    ...
    # Fallback to full load if regex on chunk fails
