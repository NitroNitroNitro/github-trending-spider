...
## 2026-07-01 - Language Switch Accessibility
**Learning:** Custom toggle controls like language switches need `aria-pressed` to explicitly communicate their active semantic state to screen readers. Purely decorative visual elements like text separators need `aria-hidden="true"` to reduce screen reader noise.
**Action:** When implementing custom toggle controls in Vue, use `aria-pressed` tied to the active state condition, and add `aria-hidden="true"` to visual separators to improve accessibility.
